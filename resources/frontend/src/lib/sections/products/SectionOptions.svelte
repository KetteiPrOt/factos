<script lang="ts">
    ...
    async function delteAllProducts () {
...
<div class="flex flex-col gap-4 justify-center border-t border-[--color-border] pt-7">
    <section class="flex flex-wrap gap-10 justify-center">
        <button class="flex flex-row gap-2 place-items-center bg-[--color-theme-1] py-1 px-2 rounded-md shadow-sm shadow-black hover:shadow hover:shadow-black hover:bg-blue-600 text-slate-50" on:click={toogleModalNewProductVisible}>
        ...
        <div class="relative">
            ...
            <div transition:slide={{axis: "y"}} class="absolute mt-1 bg-blue-700/60 backdrop-blur-sm w-full flex flex-row gap-1 p-1 text-slate-50 rounded-md">
                <button class="flex-grow border rounded shadow-sm shadow-slate-600 hover:shadow hover:shadow-slate-900 hover:bg-blue-700/30" on:click={delteAllProducts}>
                    Confirmar
                </button>
