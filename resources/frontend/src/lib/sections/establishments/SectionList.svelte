<script lang="ts">
    ...
    export let establishments: Writable<Establishments[]>;
    ...
    function selectEstablishments (e: MouseEvent) {
        ...
        if (tr.dataset.id) {
            ...
            $estabSelected = { code: 0, address: "", commercial_name: "" };
    ...
    onMount(()=>requestFunctions.loadEstablishments(undefined))
...
<section class="max-w-full">
    ...
    <div class="block w-full  rounded-md max-w-full overflow-x-auto">
        <table class="max-w-full w-full border-collapse text-center">
            ...
            <tbody>
                {#each $establishments as estab}
                <tr in:fade={{delay: ($establishments.indexOf(estab)*50)}} class="hover:bg-slate-300" data-id="{estab.id}">
                    <td>
                        <button class="w-full h-full p-2" on:click={(e)=>selectEstablishments(e)}>
                            {estab.code}
                        </button>
                    </td>
                    <td>
                        <button class="w-full h-full p-2" on:click={(e)=>selectEstablishments(e)}>
                            {estab.commercial_name}
                        </button>
                    </td>
                    <td>
                        <button class="w-full h-full p-2" on:click={(e)=>selectEstablishments(e)}>
